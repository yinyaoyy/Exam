...
public class UserServiceImpl implements UserService {
	...
	@Override
	public List<Department> getDepByList(){
		return userMapper.getDepByList();
	};
	...
	@Override
	public List<CountInfo> getCanYuLv(int examId) {
		//统计各旗县的注册的人数,以旗县为主表
		List<CountInfo> list = userMapper.getGroupArea();
		//各旗县考试的人员数据，以旗县为主表
		List<CountInfo> examList = userMapper.getExamGroupArea(examId);
		List<CountInfo> zList = new ArrayList<CountInfo>();
		for (int i = 0; i < list.size(); i++) {
			CountInfo zong = new CountInfo();
			for (int j = 0; j < examList.size(); j++) {
				if(list.get(i).getName().equals(examList.get(j).getName())){
					String ksrs = examList.get(j).getExamPerson();
					String zrs = list.get(j).getGroupTotal();
					double cyl = 0.00;
					NumberFormat nt = NumberFormat.getPercentInstance();
					nt.setMinimumFractionDigits(2);
					if(ksrs!=null&&!"".equals(ksrs)&&!"0".equals(ksrs.trim())){
						 cyl = Double.parseDouble(ksrs)/Double.parseDouble(zrs);
					}
					zong.setExamPerson(ksrs);
					zong.setGroupTotal(zrs);
					zong.setCanYuLv(nt.format(cyl));
					zong.setName(examList.get(j).getName());
					zList.add(zong);
				}
			}
