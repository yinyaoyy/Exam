...
public class ExamPage {
	...
	public String examStartPage(Model model, HttpServletRequest request, @PathVariable("examId") int examId) {
		...
		long a = exam.getExpTime().getTime();
		long b = new Date().getTime();
		long c = exam.getEffTime().getTime();
		
		if (exam.getApproved() != 1) {
			model.addAttribute("errorMsg", "<span style=\"color:#DC143C;font-weight:bold;font-size:16px\">考试未审核<span/>");
			return "error";
		}else if(b < c||b > a) {
			model.addAttribute("errorMsg", "<span style=\"color:#DC143C;font-weight:bold;font-size:16px\">当前时间不能考试<span/>");
			return "error";
		}else if(exam.getExamType() == 3) {
			model.addAttribute("errorMsg", "<span style=\"color:#DC143C;font-weight:bold;font-size:16px\">考试类型错误<span/>");
			return "error";
		}
