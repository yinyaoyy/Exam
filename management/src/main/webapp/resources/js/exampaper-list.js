...
var question_list = {
	...
	bindChangeProperty : function bindChangeProperty(){
		$(".change-property").click(function(){
			...
			var paper_field = $(this).parent().parent().find(":hidden").val();
			...
			$("#exampaper-paperType-select").val(paper_field);
	...
	bindUpdateExampaper : function bindUpdateExampaper(){
		$("#update-exampaper-btn").click(function(){
			...
			if (verify_result) {
				...
				var data = new Object();
				var examPaperId = paper_id;
				data.paperName = $(".add-update-exampapername input").val();
				data.time = $(".add-update-duration input").val();
				data.type = $("#exampaper-type-select").val();
				data.paperType = $("#exampaper-paperType-select").val();
				$.ajax({
					headers : {
						'Accept' : 'application/json',
						'Content-Type' : 'application/json'
					...
					type : "POST",
					url : util.getCurrentRole() + "/paper-update/"+examPaperId,
