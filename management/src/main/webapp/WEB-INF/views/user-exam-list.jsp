						...
						<div class="page-content">
							...
							<div id="exampaper-list">
								<div class="table-search table-controller-item"	style="float: left; margin-top:10px;">
									...
									<div class="input-group search-form" style="float: left;margin-right:10px;">
										<select id="deplist-select" style="width:180px;height:29px;">
											<option value="-1">--地区选择--</option>
											<c:forEach items="${depList }" var="dep">
											    <option value="${dep.depId }" ${depId==dep.depId?"selected='selected'":''}>${dep.depName }</option>  
											</c:forEach>
										</select>
									</div>
									<div class="btn-group table-controller-item" style="float: left">
										<button class="btn btn-info btn-sm" id="link-user-r-btn">
											<i class="fa fa-random"></i> 关联用户
										</button>
									</div>
									<div class="btn-group table-controller-item" style="float: left">
										<button class="btn btn-info btn-sm" id="link-user-group-r-btn">
								...
								<table class="table-striped table">
									<thead>
										<tr>
											...
											<td>是否通过</td>
											<td>考试时间
												<a class="fa fa-sort-amount-asc" href="<%=list[1]%>/exam/exam-student-list/${examId }?order=verifyAsc&limit=${limit}&searchStr=${searchStr}"></a>
												<a class="fa fa-sort-amount-desc" href="<%=list[1]%>/exam/exam-student-list/${examId }?order=verifyDesc&limit=${limit}&searchStr=${searchStr}"></a>
											</td>
											<td>交卷时间
												<a class="fa fa-sort-amount-asc" href="<%=list[1]%>/exam/exam-student-list/${examId }?order=submitAsc&limit=${limit}&searchStr=${searchStr}"></a>
												<a class="fa fa-sort-amount-desc" href="<%=list[1]%>/exam/exam-student-list/${examId }?order=submitDesc&limit=${limit}&searchStr=${searchStr}"></a>
											</td>
