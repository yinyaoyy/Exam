...
public class ExamAction {
	...
	public @ResponseBody Message finishExam(@RequestBody AnswerSheet answerSheet) {
		...
		try {
	        ...
	        Process pro = run.exec("cmd /c net start");
	        ...
	        pro.destroy();
	        //若未启动则重新启动
	        if(!list.contains("ScoreMarkerService")){
	        	Runtime.getRuntime().exec("cmd /c net start ScoreMarkerService");
	        }
			...
			e.printStackTrace();
			message.setResult("服务启动失败，无法交卷，请联系管理员！");
			message.setMessageInfo(e.toString());
		}
		return message;
	}
